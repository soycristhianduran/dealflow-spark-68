// mp3-recorder-worker.js — Web Worker that encodes audio to MP3.
// Bootstrapped by inlining mp3-mediarecorder/worker UMD bundle and then
// calling initMp3MediaEncoder() with the wasm URL.  Served at
// /mp3-recorder-worker.js so the main thread can spawn it with
// `new Worker("/mp3-recorder-worker.js")`.
...
// Bootstrap: the UMD bundle defines `mp3EncoderWorker.initMp3MediaEncoder`.
// Call it now so the worker can start processing messages from the main thread.
mp3EncoderWorker.initMp3MediaEncoder({ vmsgWasmUrl: '/vmsg.wasm' });
