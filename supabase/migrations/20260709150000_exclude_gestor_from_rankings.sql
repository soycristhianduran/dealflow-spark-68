-- Gestores are technical/agency staff, not sales reps (applied live 2026-07-09).
-- dashboard_extra's vendor and setter rankings now exclude any owner/setter
-- whose role in the org is 'gestor'. Both overloads patched — the legacy
-- (uuid,uuid) one and the period-aware 4-arg one the dashboard actually calls.
    ...
    'setters', (
      ...
      from (
        select c.setter_id,
          ...
          (select count(*) from meetings m join contacts cc on cc.id = m.contact_id where cc.setter_id = c.setter_id and cc.organization_id = p_org) citas,
    ...
    'vendors', (
      ...
      from (
        ...
        where c.organization_id = p_org and c.owner_id is not null
          -- Gestores (non-billable technical managers) are staff, not sales
          -- reps: never rank them in vendor performance.
          and not exists (
            select 1 from organization_members om
  ...
  result := jsonb_build_object(
    ...
    'vendors', (
      ...
      from (
        ...
        where c.organization_id = p_org and c.owner_id is not null
          and c.created_at >= v_start and c.created_at < v_end
          -- Gestores (non-billable technical staff) are not sales reps:
          -- never rank them in vendor performance.
          and not exists (
            select 1 from organization_members om
            where om.organization_id = p_org and om.user_id = c.owner_id and om.role = 'gestor')
        group by c.owner_id order by count(*) desc limit 10
