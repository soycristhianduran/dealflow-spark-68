...
const testimonials: Testimonial[] = [
  {
    quote: "En 2 semanas ya recuperamos la inversión. El lead scoring de la IA nos cambió completamente cómo priorizamos el trabajo del equipo.",
    name: "Carlos Mendoza", role: "Director Comercial", company: "AgenciaPro",
    initials: "CM", color: "bg-blue-600",
  },
  {
    quote: "Teníamos 5 herramientas distintas. Ahora todo está en Klosify y pagamos menos de la mitad. La integración con WhatsApp es impecable.",
    name: "Laura Gómez", role: "CEO", company: "VentasDigitales",
    initials: "LG", color: "bg-purple-600",
  },
  {
    quote: "Las landings con IA nos ahorran horas cada semana. Las conectamos al pipeline y los leads llegan solos, ya calificados.",
    name: "Andrés Torres", role: "Marketing Manager", company: "StartupBogotá",
    initials: "AT", color: "bg-teal-600",
  },
];
...
/** Dual-source logo with branded fallback */
...
/** Realistic CRM pipeline preview for hero */
function PipelineMockup() {
  const columns = [
    {
      label: "Nuevo", dot: "bg-slate-500", count: 2,
      leads: [
        { name: "María G.",  score: 7.4, value: "$2,400", hot: false },
        { name: "Carlos P.", score: 6.1, value: "$1,800", hot: false },
      ],
    },
    {
      label: "Calificado", dot: "bg-blue-500", count: 2,
      leads: [
        { name: "Juan M.",  score: 9.1, value: "$5,800", hot: true },
        { name: "Ana S.",   score: 8.8, value: "$3,200", hot: true },
      ],
    },
    {
      label: "Cerrado", dot: "bg-orange-500", count: 1,
      leads: [
        { name: "Luis F.", score: 9.6, value: "$8,500", hot: true },
      ],
    },
  ];
  ...
  return (
    <div className="relative">
      ...
      <div className="relative bg-slate-900 rounded-2xl border border-slate-700/80 overflow-hidden shadow-2xl shadow-slate-950/60">
        ...
        <div className="flex items-center gap-3 px-4 py-3 bg-slate-950/60 border-b border-slate-800">
          ...
          <div className="flex-1 flex justify-center">
            <div className="bg-slate-800/80 rounded px-3 py-1 text-xs text-slate-500 font-mono tracking-tight">
        ...
        <div className="flex">
          ...
          <div className="flex-1 p-4">
                    ...
                    {col.leads.map((lead) => (
                      ...
                      >
                        <div className="flex items-center justify-between mb-1">
                          ...
                          <span className={`text-xs font-bold font-mono ${lead.score >= 9 ? "text-green-400" : lead.score >= 7 ? "text-yellow-400" : "text-slate-500"}`}>
                            {lead.score}
                          </span>
                        </div>
            ...
            <div className="mt-4 pt-3 border-t border-slate-800 grid grid-cols-3 gap-3">
              {[
                { val: "$18.2k", label: "Pipeline", color: "text-white" },
                { val: "3.4×",   label: "ROAS",     color: "text-orange-400" },
                { val: "68%",    label: "Win rate",  color: "text-green-400" },
              ].map((s) => (
      ...
      <div className="absolute -right-4 top-20 bg-slate-800 border border-slate-700 rounded-xl p-3 shadow-xl shadow-slate-950/40 backdrop-blur-sm">
      ...
      <div className="absolute -left-5 bottom-20 bg-slate-800 border border-orange-500/30 rounded-xl p-3 shadow-xl shadow-slate-950/40 backdrop-blur-sm">
        <div className="flex items-center gap-2.5">
...
function PlanCard({ plan, isAnnual }: { plan: Plan; isAnnual: boolean }) {
  ...
  return (
    <div className={`relative flex flex-col rounded-2xl p-8 ${plan.popular ? "border-2 border-orange-500 shadow-2xl shadow-orange-500/10 md:scale-105 bg-white" : "border border-slate-200 bg-white shadow-sm"}`}>
      {plan.popular && (
    ...
    <div className="min-h-screen bg-white font-sans antialiased">
          ...
          <div className="flex items-center justify-between h-16">
            ...
            <div className="hidden md:flex items-center gap-8">
              {["features", "pricing"].map((id) => (
                <button key={id} onClick={() => scrollTo(id)} className="text-slate-400 hover:text-white text-sm font-medium transition-colors capitalize">
                  {id === "features" ? "Funcionalidades" : "Precios"}
                </button>
              ))}
            </div>
            ...
            <div className="hidden md:flex items-center gap-3">
              ...
              <Link to="/auth" className="bg-orange-500 hover:bg-orange-400 text-white px-4 py-2 rounded-lg text-sm font-semibold transition-all shadow-lg shadow-orange-500/25">
                Empezar gratis →
              </Link>
            </div>

            <button onClick={() => setMobileOpen(!mobileOpen)} className="md:hidden text-slate-400 hover:text-white p-2 rounded-lg" aria-label="Toggle menu">
          ...
          <div className="md:hidden border-t border-slate-800 bg-slate-950">
            <div className="px-4 py-4 space-y-2">
              {[["features", "Funcionalidades"], ["pricing", "Precios"]].map(([id, label]) => (
                <button key={id} onClick={() => scrollTo(id)} className="block w-full text-left text-slate-300 hover:text-white py-2 px-3 rounded-lg hover:bg-slate-800 text-sm font-medium">
                  {label}
                </button>
              ))}
      ...
      <section className="bg-slate-950 pt-32 pb-20 relative overflow-hidden">
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 relative">
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-14 items-center">
            ...
            {/* Left — copy */}
            <div>
              <div className="inline-flex items-center gap-2 bg-orange-500/10 text-orange-400 border border-orange-500/20 rounded-full px-4 py-1.5 text-sm font-medium mb-7">
                <Sparkles className="w-3.5 h-3.5" />
                Nuevo — IA Boost: scoring automático de leads
              </div>

              <h1 className="text-5xl lg:text-6xl xl:text-7xl font-black text-white leading-[1.06] tracking-tight">
              ...
              <div className="flex flex-col sm:flex-row items-start gap-4 mt-10">
                <Link to="/auth" className="inline-flex items-center gap-2 bg-orange-500 hover:bg-orange-400 text-white px-7 py-3.5 rounded-xl text-base font-bold transition-all shadow-xl shadow-orange-500/25 hover:shadow-orange-500/40">
                  Crear cuenta gratis
              ...
              <div className="mt-10 pt-8 border-t border-slate-800/60">
                ...
                <div className="flex items-center gap-5 flex-wrap">
                  {integrationLogos.map((logo) => (
                    <div key={logo.domain} className="grayscale opacity-50 hover:opacity-80 hover:grayscale-0 transition-all duration-300">
      ...
      <section id="features" className="bg-white py-24">
        <div className="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8">
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
            ...
            {/* Large card: WhatsApp */}
            <div className="md:col-span-2 bg-slate-950 rounded-2xl p-7 flex flex-col gap-5 overflow-hidden relative">
              ...
              {/* Mini WhatsApp thread mockup */}
              <div className="bg-slate-800/60 rounded-xl p-4 border border-slate-700/50">
                <div className="space-y-2.5">
                  {[
                    { from: "Juan M.", msg: "Hola, me interesa el plan Pro", time: "10:32", out: false },
                    { from: "Klosify Bot", msg: "¡Hola Juan! Aquí tienes el detalle del plan Pro → klosify.link/pro", time: "10:32", out: true },
                    { from: "Juan M.", msg: "Perfecto, ¿puedo hablar con alguien?", time: "10:33", out: false },
                  ].map((m, i) => (
            ...
            {/* Normal card: Meta Ads */}
            <div className="bg-slate-950 rounded-2xl p-7 flex flex-col gap-4 relative overflow-hidden">
              ...
              <div>
                ...
                <p className="text-slate-400 text-sm leading-relaxed">Conecta Facebook e Instagram Ads y ve el retorno real de cada peso invertido.</p>
              </div>
              {/* Mini bar chart */}
              <div className="flex items-end gap-1.5 h-16 mt-auto">
                {[40, 65, 45, 80, 55, 90, 70].map((h, i) => (
                  <div key={i} className="flex-1 rounded-sm" style={{ height: `${h}%`, background: i === 5 ? "#f97316" : "#334155" }} />
                ))}
              </div>
              <div className="flex items-center justify-between">
            ...
            <div className="bg-slate-950 rounded-2xl p-7 flex flex-col gap-4 relative overflow-hidden">
              ...
              <div>
                ...
                <p className="text-slate-400 text-sm leading-relaxed">Prioriza los leads con mayor probabilidad de cierre automáticamente.</p>
              </div>
              {/* Score indicators */}
              <div className="space-y-2 mt-auto">
                {[
                  { name: "Juan M.", score: 9.1, color: "bg-green-500" },
                  { name: "Ana S.",  score: 8.8, color: "bg-green-500" },
                  { name: "Pedro R.", score: 6.2, color: "bg-yellow-500" },
                ].map((l) => (
                  <div key={l.name} className="flex items-center gap-3">
                    <div className="flex-1 bg-slate-800 rounded-full h-1.5 overflow-hidden">
                      <div className={`h-full ${l.color} rounded-full`} style={{ width: `${l.score * 10}%` }} />
                    </div>
            ...
            {/* Large card: Pipeline */}
            <div className="md:col-span-2 bg-slate-950 rounded-2xl p-7 flex flex-col gap-5 relative overflow-hidden">
            ...
            {/* Highlight card: social proof */}
            <div className="bg-gradient-to-br from-orange-500 to-orange-600 rounded-2xl p-7 flex flex-col gap-3">
              ...
              <Link to="/auth" className="inline-flex items-center gap-1.5 text-white font-semibold text-sm mt-2 hover:gap-2.5 transition-all">
      ...
      <section className="bg-slate-950 py-24">
        <div className="max-w-5xl mx-auto px-4 sm:px-6 lg:px-8">
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-8 relative">
            {/* Connecting line */}
            <div className="hidden md:block absolute top-10 left-[calc(33%+1rem)] right-[calc(33%+1rem)] h-px bg-gradient-to-r from-orange-500/40 via-orange-500/20 to-orange-500/40" />
            ...
            {[
              ...
              <div key={step} className="relative">
                <div className="w-14 h-14 bg-slate-900 border border-orange-500/30 rounded-2xl flex flex-col items-center justify-center mb-6 relative z-10">
        ...
        <div className="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="text-center mb-14">
            ...
            <div className="inline-flex items-center bg-white border border-slate-200 rounded-xl p-1 gap-1 shadow-sm">
              <button onClick={() => setIsAnnual(false)} className={`px-5 py-2 rounded-lg text-sm font-semibold transition-all ${!isAnnual ? "bg-slate-900 text-white shadow-sm" : "text-slate-500 hover:text-slate-700"}`}>
                Mensual
              </button>
              <button onClick={() => setIsAnnual(true)} className={`flex items-center gap-2 px-5 py-2 rounded-lg text-sm font-semibold transition-all ${isAnnual ? "bg-slate-900 text-white shadow-sm" : "text-slate-500 hover:text-slate-700"}`}>
                Anual
                <span className="bg-green-500 text-white text-xs px-1.5 py-0.5 rounded-full font-bold">2 meses gratis</span>
              </button>
            </div>
          </div>
          ...
          <div className="mt-10">
            ...
            <div className="grid grid-cols-2 md:grid-cols-4 gap-4 max-w-3xl mx-auto">
              {addOns.map((addon) => {
                ...
                return (
                  <div key={addon.label} className="bg-white rounded-xl p-5 text-center border border-slate-200 hover:border-orange-200 hover:shadow-md transition-all">
      ...
      <section className="bg-slate-950 py-24">
        <div className="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8">
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
            {testimonials.map((t) => (
              <div key={t.name} className="bg-slate-900 border border-slate-800 rounded-2xl p-7 flex flex-col gap-5">
      ...
      <section className="bg-slate-950 py-24 text-center relative overflow-hidden">
        ...
        <div className="relative max-w-3xl mx-auto px-4 sm:px-6">
          ...
          <Link to="/auth" className="inline-flex items-center gap-2 bg-orange-500 hover:bg-orange-400 text-white px-10 py-4 rounded-xl text-base font-bold transition-all shadow-2xl shadow-orange-500/30 hover:shadow-orange-500/50 hover:-translate-y-0.5">
