...
interface Insights {
  ...
  vendors: { owner_id: string; leads: number; citas: number }[];
}
...
export function DashboardInsights({ stageData, isOwner, vendorId }: { stageData: StageDatum[]; isOwner: boolean; vendorId: string | null }) {
          ...
          <Card className="border-none shadow-sm">
            ...
            <CardContent className="space-y-2">
              {data.vendors.length === 0 ? (
                ...
                <div key={v.owner_id} className="flex items-center gap-3 rounded-lg border p-2.5">
                  ...
                  <span className="text-sm flex-1 truncate">{vendorNames[v.owner_id] || "Vendedor"}</span>
                  <span className="text-xs text-muted-foreground"><b className="text-foreground">{v.leads.toLocaleString()}</b> leads</span>
                  <span className="text-xs text-muted-foreground"><b className="text-foreground">{v.citas}</b> citas</span>
                </div>
              ))}
            </CardContent>
          </Card>
